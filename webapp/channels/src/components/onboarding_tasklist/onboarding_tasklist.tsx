...
const OnBoardingTaskList = (): JSX.Element | null => {
  const hasPreferences = useSelector(
    (state: GlobalState) =>
      Object.keys(getMyPreferencesSelector(state)).length !== 0
  );
  ...
  const isEnableOnboardingFlow = useSelector(
    (state: GlobalState) => getConfig(state).EnableOnboardingFlow === "true"
  );
            ...
            <>
              <h1>
                <FormattedMessage
                  ...
                  defaultMessage="Welcome to Nimbupani"
                />
              </h1>
              <p>
                <FormattedMessage
